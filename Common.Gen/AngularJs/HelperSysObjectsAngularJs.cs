...
{
    public class HelperSysObjectsAngularJs : HelperSysObjectsBase
    {

        private DefineTemplateFolder _defineTemplateFolder;
        public HelperSysObjectsAngularJs(Context context)
        {
        ...
        {
            ExecuteTemplateAngularModalCreate(tableInfo, config, infos);
            ExecuteTemplateAngularModalEdit(tableInfo, config, infos);
            ExecuteTemplateAngularDetails(tableInfo, config, infos);
            ExecuteTemplateAngularFormCreate(tableInfo, config, infos);
            ExecuteTemplateAngularFormEdit(tableInfo, config, infos);
            ExecuteTemplateAngularDetailsFields(tableInfo, config, infos);
            ExecuteTemplateAngularEditInPage(tableInfo, config, infos);
            ExecuteTemplateAngularCreateInPage(tableInfo, config, infos);
            ExecuteTemplateAngularView(tableInfo, config, infos);
            ExecuteTemplateAngularViewFilters(tableInfo, config, infos);
            ExecuteTemplateAngularViewGrid(tableInfo, config, infos);
            ExecuteTemplateAngularLabelsConstants(tableInfo, config, infos);
        }
        ...
        private string getEditFunc(TableInfo tableInfo, Context configContext)
        {
            ...
            {
                getEditType = @"EditByFilter(" + MakeComposedKeysFilter(tableInfo, configContext.CamelCasing) + ")";
            }
        ...
        private string getDetailsFunc(TableInfo tableInfo, Context configContext)
        {
            ...
            if (tableInfo.IsCompositeKey)
            {
                getDetailsType = @"DetailsByFilter(" + MakeComposedKeysFilter(tableInfo, configContext.CamelCasing) + ")";
            }
        ...
        private string getPrintFunc(TableInfo tableInfo, Context configContext)
        {
            ...
            if (tableInfo.IsCompositeKey)
            {
                getDetailsType = @"PrintByFilter(" + MakeComposedKeysFilter(tableInfo, configContext.CamelCasing) + ")";
            }
        ...
        private string getDeleteFunc(TableInfo tableInfo, Context configContext)
        {
            ...
            if (tableInfo.IsCompositeKey)
            {
                getDetailsType = @"Delete(" + MakeComposedKeysFilter(tableInfo, configContext.CamelCasing) + ")";
            }
        ...
        private void ExecuteTemplateAngularFormCreate(TableInfo tableInfo, Context configContext, IEnumerable<Info> infos)
        {
            ...
            if (infos.IsAny())
            {
                foreach (var item in infos)
                {
                    ...
                    var propertyName = configContext.CamelCasing ? CamelCasing(item.PropertyName) : item.PropertyName;

                    var fieldInBlackListCreate = FieldInBlackListCreate(tableInfo, propertyName);
                    ...
                    if (item.Type == "string")
                    {
                        var str = HelperControlHtml.MakeInputHtml(tableInfo, item, isStringLengthBig);
                    ...
                    else
                    {
                        ...
                        {
                            var isSelectSearch = IsSelectSearch(tableInfo, propertyName);
                            if (isSelectSearch)
                            {
                    ...
                    var htmlCtrl = FieldHtml(tableInfo, propertyName);
                    ...
                    var isUpload = IsUpload(tableInfo, propertyName);
                    if (isUpload)
                    {
        ...
        private string FormFieldReplace(Context configContext, TableInfo tableInfo, Info info, string textTemplateForm, string str)
        {
            var cols = "col-md-12";
            if (tableInfo.twoCols)
                cols = "col-md-6";
        ...
        private void ExecuteTemplateAngularFormEdit(TableInfo tableInfo, Context configContext, IEnumerable<Info> infos)
        {
            ...
            if (infos.IsAny())
            {
                foreach (var item in infos)
                {
                    ...
                    var fieldInBlackListEdit = FieldInBlackListEdit(tableInfo, propertyName);
        ...
        private static bool FieldInBlackListCreate(TableInfo tableInfo, string propertyName)
        {
        ...
        private static HtmlCtrl FieldHtml(TableInfo tableInfo, string propertyName)
        {
            if (tableInfo.FieldsConfig.NotIsAny())
                return null;
        ...
        private static bool IsUpload(TableInfo tableInfo, string propertyName)
        {
            if (tableInfo.FieldsConfig.NotIsAny())
                return false;
        ...
        private static bool IsSelectSearch(TableInfo tableInfo, string propertyName)
        {
            if (tableInfo.FieldsConfig.NotIsAny())
                return false;
        ...
        private static bool IsTextEditor(TableInfo tableInfo, string propertyName)
        {
            if (tableInfo.FieldsConfig.NotIsAny())
                return false;
        ...
        private static Dictionary<string, string> FieldDataItem(TableInfo tableInfo, string propertyName)
        {
            if (tableInfo.FieldsConfig.NotIsAny())
        ...
        private static bool FieldInBlackListEdit(TableInfo tableInfo, string propertyName)
        {
            if (tableInfo.FieldsConfig.NotIsAny())
                return false;
            ...
            return tableInfo.FieldsConfig
                .Where(_ => _.Name.ToUpper() == propertyName.ToUpper())
                .Where(_ => _.Edit == false)
                .IsAny();
        }

        private static bool FieldInBlackListDetails(TableInfo tableInfo, string propertyName)
        {
            if (tableInfo.FieldsConfig.NotIsAny())
                return false;
            ...
            return tableInfo.FieldsConfig
                .Where(_ => _.Name.ToUpper() == propertyName.ToUpper())
                .Where(_ => _.Details == false)
                .IsAny();
        }

        private static bool FieldInBlackListFilter(TableInfo tableInfo, string propertyName)
        {
            if (tableInfo.FieldsConfig.NotIsAny())
                return false;
        ...
        private static bool FieldInBlackListGrid(TableInfo tableInfo, string propertyName)
        {
            if (tableInfo.FieldsConfig.NotIsAny())
                return false;
        ...
        private void ExecuteTemplateAngularDetailsFields(TableInfo tableInfo, Context configContext, IEnumerable<Info> infos)
        {
            ...
            if (infos.IsAny())
            {
                foreach (var item in infos)
                {
                    ...
                    var itemForm = string.Empty;
                    var propertyName = configContext.CamelCasing ? CamelCasing(item.PropertyName) : item.PropertyName;
                    var fieldInBlackListDetails = FieldInBlackListDetails(tableInfo, propertyName);
                    ...
                    var dataitem = FieldDataItem(tableInfo, propertyName);
                    if (dataitem.IsAny())
                    {
        ...
        private void ExecuteTemplateAngularViewFilters(TableInfo tableInfo, Context configContext, IEnumerable<Info> infos)
        {
            ...
            if (infos.IsAny())
            {
                foreach (var item in infos)
                {
                    ...
                    var propertyName = configContext.CamelCasing ? CamelCasing(item.PropertyName) : item.PropertyName;
                    var fieldInBlackListFilter = FieldInBlackListFilter(tableInfo, propertyName);
        ...
        private void ExecuteTemplateAngularViewGrid(TableInfo tableInfo, Context configContext, IEnumerable<Info> infos)
        {
            ...
            if (infos.IsAny())
            {
                foreach (var item in infos)
                {
                    ...
                    var propertyName = configContext.CamelCasing ? CamelCasing(item.PropertyName) : item.PropertyName;
                    var fieldInBlackListGrid = FieldInBlackListGrid(tableInfo, propertyName);
                    ...
                    if (item.Type == "int" || item.Type == "int?")
                    {
                        if (!IsPropertyInstance(tableInfo, item.PropertyName))
                        {
                            if (item.IsKey != 1 || configContext.ShowKeysInFront)
                            {
                                itemTableFieldsHead = textTemplateTableHead.Replace("<#propertyName#>", propertyName);
        ...
        protected override string GenericTagsTransformer(TableInfo tableInfo, Context configContext, string classBuilder)
        {

            var keyName = tableInfo.Keys.IsAny() ? tableInfo.Keys.FirstOrDefault() : string.Empty;
            if (configContext.CamelCasing)
                keyName = CamelCasing(keyName);

            classBuilder = classBuilder.Replace("<#KeyName#>", keyName);

            return base.GenericTagsTransformer(tableInfo, configContext, classBuilder);
        }
